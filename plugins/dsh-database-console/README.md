...
- 会话里“说人话”查库：连接参数支持 id/名称/**口语别名**（“36 数据库”→ 主机 192.168.48.36），
  `database` 参数支持同服务器多库切换；系统提示注入连接目录与用法规则
...
普通对话中不必背连接 id。例如用户说“查一下 36 数据库 test2 里的 Item 表”，
AI 会理解为：连接「48.36」（主机 192.168.48.36，默认库 `test2`）→ `Item` 表，然后只读查询。

- `connection` 参数 = 连接 **id / 名称 / 口语别名**（别名取连接名与主机的段、点分后缀，如 “36”/“48.36”/“192.168.48.36”）。
  同分多条命中时报歧义并列出候选，不猜错库。
- `db_connections`：连接目录，每条附主机、默认库/模式与“可写作”的别名。
- `db_databases`：PG/MySQL 服务器级库列表；目标库不是连接默认库时，把 `database` 传给 `db_tables`/`db_table_schema`/`db_query` 切换。
- `schema`：PostgreSQL / 达梦的模式(owner)；MySQL 的库与 schema 同义。
- SQLite（单文件）与达梦不支持跨库切换，工具会明确报错。
- 系统提示注入“启动时连接目录快照”；新增连接后以 `db_connections` 返回为准。
