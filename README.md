...
- SQL 控制台：多语句、参数绑定、只读模式（默认开启，写操作需显式取消）；
  结果**统计总行数并支持服务端分页**（上一页/下一页）：只读单条 SELECT/WITH 由服务端包装
  COUNT 统计总数、下推 LIMIT/OFFSET 取页（达梦 oracle 兼容自动改用 rownum Top-N；自带 LIMIT
  的语句包一层派生表再翻页；MongoDB JSON 查询用 skip/countDocuments），写语句/多条/SHOW 等
  自动回退为整语句执行
- 查询历史：每个连接保留最近 50 条 SQL / 自然语言查询记录（存浏览器 localStorage，
  成功执行的语句/问题自动去重置顶），在 SQL 查询与自然语言查询子页点击条目即可载入
